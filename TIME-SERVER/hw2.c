...
{
    #define PORT_NUMBER 60000 // the port number that will be used by program
    ...
    server.sin_port = htons(PORT_NUMBER); // defining port number (60000)
           ...
           accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c) )
    {
	    ...
	    else
	    {
			...
			//write nbyte bytes from the buffer pointed to by buf to the file associated with the open file descriptor, fildes.
  			...
  			strcat(time_date,":");
  			strcat(time_date,min);
  			strcat(time_date,":");
  			strcat(time_date,sec);
  			strcat(time_date,", ");
  			...
  			strcat(time_date,".");
  			strcat(time_date,mon);
  			strcat(time_date,".");
  			strcat(time_date,year);
  			strcat(time_date,"\n");
  			...
  			write(new_socket,time_date,strlen(time_date));
	        	...
	        	else if(day_of_week==1) strcat(wday,"Monday");
	        	else if(day_of_week==2) strcat(wday,"Tuesday");
	        	else if(day_of_week==3) strcat(wday,"Wednesday");
	        	else if(day_of_week==4) strcat(wday,"Thursday");
	        	else if(day_of_week==5) strcat(wday,"Friday");
	        	else if(day_of_week==6) strcat(wday,"Saturday");
	        	else if(day_of_week==7) strcat(wday,"Sunday");
	        	strcat(result,wday);
	        	strcat(result,"\n");
	        	//write nbyte bytes from the buffer pointed to by buf to the file associated with the open file descriptor, fildes.
	        	write(new_socket,result,strlen(result));
